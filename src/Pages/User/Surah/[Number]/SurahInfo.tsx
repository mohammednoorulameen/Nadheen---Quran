...
export default function SurahInfoPage() {
  ...
  useEffect(() => {
    if (!surahNum || Number.isNaN(surahNum)) return

    const BASE_URL =
      import.meta.env.VITE_API_BASE_URL || "https://api.alquran.cloud/v1"

    setLoading(true)

    fetch(`${BASE_URL}/surah/${surahNum}`)
      .then((res) => res.json())
      .then((res) => {
        if (res?.data) {
          setMeta(res.data)
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 pb-24">
      ...
      <div className="mt-6 space-y-2 text-sm">
        <p>
          <strong>Ayahs:</strong> {meta.numberOfAyahs}
        </p>
        <p>
          <strong>Revelation:</strong> {meta.revelationType}
        </p>
      </div>
    </main>
  )
