...
// async function getSurah(number: string) {
//   const n = Number(number)
//   if (!Number.isFinite(n) || n < 1 || n > 114) return null

//   // Fetch Arabic Uthmani and English (Sahih) in parallel
//   const [arRes, enRes] = await Promise.all([
//     fetch(`https://api.alquran.cloud/v1/surah/${n}/quran-uthmani`, { next: { revalidate: 3600 } }),
//     fetch(`https://api.alquran.cloud/v1/surah/${n}/en.sahih`, { next: { revalidate: 3600 } }),
//   ])
//   if (!arRes.ok) return null
//   const arJson = await arRes.json()
//   const arData = arJson?.data
...
//   let translatorName: string | undefined
//   let enAyahs: Array<{ numberInSurah: number; text: string }> = []
//   if (enRes.ok) {
//     const enJson = await enRes.json()
//     const enData = enJson?.data
//     translatorName = enData?.edition?.englishName ?? enData?.edition?.name
//     enAyahs = (enData?.ayahs ?? []).map((a: any) => ({
...
//   const arAyahs: Ayah[] = (arData.ayahs ?? []).map((a: any) => ({
//     number: a.number,
//     numberInSurah: a.numberInSurah,
//     juz: a.juz,
//     page: a.page,
...
//   // Join Arabic + English by numberInSurah
//   const verses: Verse[] = arAyahs.map((a) => {
//     const en = enAyahs.find((e) => e.numberInSurah === a.numberInSurah)
//     return {
//       numberInSurah: a.numberInSurah,
...
//   const data = await getSurah(params.number)
//   if (!data) {
//     return { title: "Surah Not Found" }
//   }
//   return {
//     title: `Surah ${data.meta.englishName} (${data.meta.name})`,
//     description: `Read Surah ${data.meta.englishName} (${data.meta.name}) with ${data.meta.numberOfAyahs} verses.`,
//   }
// }
...
//   const center = params.number === "2"
//   const startAtTop =
//     (typeof searchParams?.start === "string" && searchParams?.start === "top") ||
//     (Array.isArray(searchParams?.start) && searchParams?.start[0] === "top")
